...
__global__ void radixSort(int *inp, int *out, int *bits, int iter)
{
    ...
    if (threadIdx.x == 0) 
    {
        ...
        if (last_idx < N) 
        {
            last_bit = (inp[last_idx] >> iter) & 1;
        }
        oneTotal = bits[last_idx] + last_bit;
    }
    __syncthreads();
...
int main() 
{
    int *a, *output_cpu, *output_gpu, *output_gpu_hybrid, *bits;
    size_t size = N * sizeof(int);

    cudaMallocManaged(&a, size);
    cudaMallocManaged(&output_gpu, size);
    cudaMallocManaged(&output_gpu_hybrid, size);
    ...
    cudaMemset(output_gpu_hybrid, 0, size);
    ...
    cudaEventCreate(&stop);
    cudaEventRecord(start);
    radixSortShared<<<BLOCKS, THREADS>>>(a, output_gpu_hybrid, bits, iter);
    cudaDeviceSynchronize();
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);

    float gpu_hybrid_time;
    cudaEventElapsedTime(&gpu_hybrid_time, start, stop);
    cudaEventDestroy(start);
    cudaEventDestroy(stop);

    printf("CPU execution time: %.4f ms\n", cpu_time);
    printf("GPU execution time: %.4f ms\n", gpu_time);
    printf("GPU execution time (Shared): %.4f ms\n", gpu_hybrid_time);

    cudaFree(a);
    cudaFree(output_gpu);
    cudaFree(output_gpu_hybrid);
    cudaFree(bits);
    free(output_cpu);
