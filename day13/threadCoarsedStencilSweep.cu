...
#define IN_TILE_COARSED 32
#define OUT_TILE_COARSED (IN_TILE_COARSED - 2 * ORDER)
...
__global__ void verify(float *output_cpu, float *output_gpu, float *output_gpu_tiled, float *output_gpu_coarsed, int N, int *errors)
{
    int depth = blockIdx.z * blockDim.z + threadIdx.z;
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (depth >= 1 && depth < N - 1 && row >= 1 && row < N - 1 && col >= 1 && col < N - 1)
    {
        if ((fabs(output_cpu[depth * N * N + row * N + col] - output_gpu[depth * N * N + row * N + col]) > 1e-4) && (fabs(output_gpu[depth * N * N + row * N + col] - output_gpu_tiled[depth * N * N + row * N + col]) > 1e-4) && (fabs(output_gpu_tiled[depth * N * N + row * N + col] - output_gpu_coarsed[depth * N * N + row * N + col]) > 1e-4))
        {
            atomicAdd(errors, 1);
        }
    }
...
{
    int N = 256;
    float *input, *output_gpu, *output_gpu_tiled, *output_gpu_coarsed, *output_cpu;
    ...
    cudaMallocManaged(&output_gpu_coarsed, size);
    ...
    float gpu_time, gpu_time_tiled, gpu_time_coarsed, cpu_time;
    ...
    threadCoarsedStencilGPU<<<blocks, threads>>>(input, output_gpu_coarsed, val, N);
    cudaDeviceSynchronize();

    cudaEventRecord(stop3);
    cudaEventSynchronize(stop3);
    cudaEventElapsedTime(&gpu_time_coarsed, start3, stop3);
    ...
    verify<<<blocks, threads>>>(output_cpu, output_gpu, output_gpu_tiled, output_gpu_coarsed, N, errors);
    cudaDeviceSynchronize();

    printf("Number of mismatches: %d\n", *errors);
    printf("GPU time: %f ms\n", gpu_time);
    printf("Tiled GPU time %f ms\n", gpu_time_tiled);
    printf("Thread Coarsed GPU time: %f ms\n", gpu_time_coarsed);
    printf("CPU time: %f ms\n", cpu_time);

    cudaFree(input);
    cudaFree(output_gpu);
    cudaFree(output_gpu_tiled);
    cudaFree(output_gpu_coarsed);
    cudaFree(output_cpu);
    cudaFree(val);
    cudaFree(errors);
